...
> Compatibilité site web : prévoir dès maintenant que chaque fiche d'œuvre contienne ce qu'une publication en boutique demanderait (titre, description, prix, image, statut), pour que le module de la phase 5 s'ajoute plus tard sans tout refaire.
...
**Portée actuelle : phases 1 à 3 (le logiciel de gestion).** Les phases 4 et 5 sont des modules futurs, à bâtir une fois le logiciel de gestion en service : la phase 4 touche la comptabilité et Sage 50, la phase 5 la publication vers le site web.
...
**Phase 5 : Publication vers le site web (module futur et optionnel)**
Permettre de publier une œuvre vers la boutique du site depuis l'application, et de la marquer comme vendue ou retirée au moment de la vente, sans ressaisie. Le site semble fonctionner sous WordPress et WooCommerce (adresses en /produit/ et /boutique/), qui offre une voie d'intégration officielle par son interface de programmation (clés d'accès à fournir par la personne qui gère le site). Règles : seules des informations publiques de catalogue (titre, description, prix, image, statut) traversent vers le web ; aucune donnée de client ni de vente ne sort de l'ordinateur. Garder ce module séparé et optionnel, pour que le cœur de l'application reste simple et fonctionne hors ligne. À confirmer (plateforme et accès) avant de bâtir.
...
Approche en plusieurs couches, pensée pour des propriétaires non techniciens et un ordinateur qui peut se trouver dans un lieu de passage.

- **Chiffrement du disque.** Activer BitLocker (intégré à Windows) pour qu'un ordinateur volé soit illisible. À jumeler avec un vrai mot de passe de session Windows et un verrouillage automatique de l'écran après inactivité.
- **Base de données chiffrée.** L'application chiffre son propre fichier de base, de sorte qu'une copie du fichier soit inutilisable sans la clé. Déverrouillage invisible : la clé est rangée dans le coffre sécurisé de Windows, lié à la session de l'utilisateur, sans mot de passe maître à saisir chaque jour.
- **Verrou léger sur l'application.** Un code court pour ouvrir l'app, ou un verrouillage automatique après inactivité, pour empêcher une personne de passage de consulter les fiches de clients.
- **Clés d'accès au coffre.** Les futures clés (Sage, site web) ne sont jamais écrites en clair ; elles sont confiées au coffre sécurisé du système.
- **Sauvegardes protégées.** Les sauvegardes reçoivent la même protection que la base : conservées sur le disque chiffré, copiées périodiquement sur une clé USB elle-même chiffrée et gardée hors site, avec au moins une copie tenue hors ligne (protection contre les rançongiciels).
- **Hygiène contre les intrusions.** Windows et l'antivirus à jour, compte sans droits d'administrateur au quotidien, méfiance envers les courriels piégés.
- **Moins de données.** Ne conserver que le nécessaire et pouvoir supprimer une fiche client, ce qui réduit le risque et l'impact d'une fuite (principe de la Loi 25).

Option future : si la galerie veut une protection maximale, on pourra passer à un mot de passe maître qui déverrouille la base. Réglage réversible.

## 11. Page de réglages

Une page de réglages accessible depuis l'écran d'accueil, sobre et lisible, organisée en quatre sections. Les réglages sont enregistrés dans un fichier de configuration local (distinct de la base de données) et relus à chaque démarrage.

**La galerie**
Nom, adresse, téléphone, courriel, site web, numéro de TPS, numéro de TVQ, logo (chemin vers le fichier image). Ces informations alimentent automatiquement les bas de page, les en-têtes et les blocs de coordonnées de tous les documents générés. Modifier ici met à jour tous les futurs documents sans toucher aux gabarits.

**Les documents**
- Préfixe et numéro de départ des factures (exemple : F-2026-001).
- Taux de TPS (défaut : 5 %) et taux de TVQ (défaut : 9,975 %), chacun avec un interrupteur pour les activer ou non.
- Pourcentage de cote par défaut de la galerie (prérempli à la création d'une facture artiste, modifiable par vente).
- Texte du signataire sur le certificat d'authenticité.
...
**La sécurité**
- Activation du verrou léger sur l'application (oui/non).
- Délai d'inactivité avant verrouillage automatique (exemple : 5, 10 ou 15 minutes).
- Gestion du code de déverrouillage (définir ou modifier).

> Remarque pour Claude Code : les coordonnées de la galerie et les taux de taxes sont actuellement écrits en dur dans les gabarits HTML des documents. Dès que la page de réglages est en place, remplacer ces valeurs fixes par les données lues depuis la configuration.
