...
**▶ Prochaine étape (feuille de route convenue le 2026-06-30)** — ordre : **① reproductions ✓** · **② Sécurité** ← *en cours* · **③ Phase 5 — Web (WooCommerce)** · **④ Phase 4 — Sage 50**. *(Phases 4 et 5 inversées sur demande de Dave.)* La **② Sécurité** est **impérative** et autonome, en deux volets. **Volet 1 — verrou léger : ✓ BÂTI (2026-06-30, à confirmer par Dave dans l'app)** : code NIP 4–6 chiffres haché scrypt+sel (jamais en clair, vérif côté main `src/securite.js`, empreinte retirée de `config:get`), écran à pavé numérique `src/app/verrou.js` (verrou au démarrage + inactivité réglable 0/5/10/15/30 min + option au blur), carte **Réglages → Sécurité**, app rendue `inert` pendant le verrou. IPC `securite:*`, défauts `securite.*`, démo `demos/verrou-securite.html`, tests unitaires OK. **Volet 2 — chiffrement de la base : à faire**, décision = **A + B** : chiffrer le fichier au repos via **safeStorage/DPAPI** (réutilise le patron de la clé IA ; checkpoint WAL + chiffrer à la fermeture, déchiffrer au démarrage avant `openDatabase`, récupération après plantage en préférant le `.db` clair plus récent, chiffrer aussi les sauvegardes, interrupteur Réglages) **+ BitLocker documenté** en complément disque (photos/PDF). Écarté : SQLCipher (casse la décision #1 `node:sqlite`). **Facture client** parquée jusqu'à la Phase 4 (son n°/rôle dépend de Sage). Pour Sage : **jamais d'écriture directe** (fichier d'import + lecture ODBC, cf. CLAUDE.md §8). Réglés et retirés de la liste : lettre de remerciement, cotes, documents, nomenclature. Modèle éco : reste à valider avec les parents la liste complète des types d'œuvre + % (le reste du modèle éco est réglé). Détail dans « Plan structuré » + `A-VALIDER.md`.
