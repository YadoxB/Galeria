...
- **Lot 4 — refonte de la section Documents** — la Section Documents (#15) ne liste que certificats + factures artiste. **Étendre** pour regrouper et ranger clairement **tous les types** : certificats, factures artiste, **catalogues**, **annexes A**, **présentations**, **rapports**, **pochettes**. **🟡 Démo validée, intégration en pause** (`demos/documents-refonte.html`). **Décisions arrêtées avec Dave (2026-06-19)** :
  - **Groupement par type** (sections : Certificats, Factures artiste, Catalogues, Annexes, Présentations, Rapports, Pochettes), avec sous-séparateurs d'année si le type couvre plusieurs années.
  - **Barre de filtres permanente** (pas un panneau à ouvrir) : **Type · Année · Artiste · Client** + recherche + « Réinitialiser ».
  - **Vraies icônes SVG** par type ; annexe **dépôt rouge** / **retrait bleu**.
  - **Présentations incluses** comme type ; **pochettes dépliables** (listent leur contenu, « Ouvrir le dossier ») ; **badge « version modifiée »**.
  - **Source de données hybride** : requêtes base (certificats, factures artiste, annexes, présentations — métadonnées riches) **+ scan disque** des sous-dossiers `Documents\{année}\{Type}\` pour catalogues, rapports, pochettes et les **« versions modifiées »** (non enregistrées en base).
  - **Reste à coder** : (1) IPC `documents:liste` enrichi + scan disque, (2) nouvelle vue `documents.js`, (3) CSS. Re-générer limité aux types base ; catalogues/rapports/versions modifiées = Voir + Dossier.
...
- **Tutoriel de première ouverture (#13)** — *à faire* : overlay guidé au 1er lancement (détection via marqueur, cf. `.seed-applique`). **Le point d'entrée existe déjà** : le bouton « Revoir le tutoriel » dans la section d'aide (#22) affiche « à venir » et n'aura qu'à lancer l'overlay une fois bâti.
