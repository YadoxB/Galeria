...
**Tout le travail (verrou + retours d'usage + dossier de données + 4 outils complémentaires) vit maintenant sur `master`, en une seule ligne d'histoire.** `package.json` = `0.10.0` (inchangé ; on numérotera à la publication). **Les parents sont toujours sur une version antérieure — rien ne leur est parvenu : AUCUNE release n'a été publiée** (`npm run release` jamais lancé).
