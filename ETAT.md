...
- **Lot 3** (en cours) : **pochette de vente**. Architecture : présentation artiste = document **autonome mis en cache** (réutilisé tant que le profil ne change pas, signature SHA1 des champs), réutilisée par la pochette ; lettre+œuvre = document par vente (8 variantes FR/EN × personne/web × cadeau).
  ...
  - **Reste — Étape 3** : gabarit lettre de remerciement + fiche œuvre (8 lettres du brief). **Étape 4** : bouton « Produire la pochette » sur la vente (lettre+œuvre + présentation réutilisée + rappel certificat + `guide_certificat.pdf` déjà copié dans `gabarits/actifs/`).
  - Note : « distinctions » **intégré au CV** (pas de champ séparé), démarche incluse dans la présentation.
