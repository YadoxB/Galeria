...
**▶ Prochaine étape (feuille de route convenue le 2026-06-30)** — ordre : **① reproductions ✓** · **② Sécurité** ← *en cours* · **③ Phase 5 — Web (WooCommerce)** · **④ Phase 4 — Sage 50**. *(Phases 4 et 5 inversées sur demande de Dave.)* La **② Sécurité** est **impérative** et autonome, en deux volets. **Volet 1 — verrou léger : ✓ CONFIRMÉ par Dave dans l'app (2026-06-30)** : code NIP 4–6 chiffres haché scrypt+sel (jamais en clair, vérif côté main `src/securite.js`, empreinte retirée de `config:get`), écran à pavé numérique `src/app/verrou.js` (verrou au démarrage + inactivité réglable 0/5/10/15/30 min + option au blur), carte **Réglages → Sécurité**, app rendue `inert` pendant le verrou. IPC `securite:*`, défauts `securite.*`, démo `demos/verrou-securite.html`, tests unitaires OK. **Volet 2 — chiffrement de la base : ✓ BÂTI (2026-06-30, à confirmer par Dave dans l'app)**, décision = **A + B** : option opt-in (Réglages → Sécurité) qui chiffre le fichier au repos via **safeStorage/DPAPI** (`src/db/chiffrement.js`) — app fermée → `galerie.db.enc` ; déchiffrement au démarrage **avant** `openDatabase`, re-chiffrement + suppression du clair (+ wal/shm) à la fermeture, écriture atomique, **clair résiduel après plantage fait foi** (aucune perte). **Sauvegardes EN CLAIR** (décision Dave : récupération d'abord ; protégées par **BitLocker** + USB chiffrée, §10). Message clair si coffre inaccessible. IPC `chiffrement:*`, défaut `securite.chiffrement_actif`, tests unitaires OK. Écarté : SQLCipher (casse la décision #1 `node:sqlite`). **Pendant l'exécution la base est en clair sur le disque** (limite node:sqlite, couverte par BitLocker). **Reste côté Dave : activer BitLocker** sur le PC des parents (procédure CLAUDE.md §10). **Bonus récupération : bouton « Restaurer une sauvegarde… »** (Réglages → Sauvegardes) — valide le fichier + montre son contenu, copie de sécurité de l'actuelle, remplace, redémarre (IPC `backup:choisir-restauration`/`backup:restaurer`, helper `inspecterFichierBase`) ; complète le scénario sinistre (restaurer une sauvegarde en clair sur un PC neuf). **➜ Phase Sécurité quasi complète** une fois le volet 2 + la restauration confirmés dans l'app. **Facture client** parquée jusqu'à la Phase 4 (son n°/rôle dépend de Sage). Pour Sage : **jamais d'écriture directe** (fichier d'import + lecture ODBC, cf. CLAUDE.md §8). Réglés et retirés de la liste : lettre de remerciement, cotes, documents, nomenclature. Modèle éco : reste à valider avec les parents la liste complète des types d'œuvre + % (le reste du modèle éco est réglé). Détail dans « Plan structuré » + `A-VALIDER.md`.
