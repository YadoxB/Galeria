...
- **Lot 2** (catalogue ✓ fait) : **catalogue artiste** livré — bouton « Catalogue PDF » sur la fiche artiste → `gabarit-catalogue.html` (portrait, 6 œuvres/page), `genererCataloguePdf` (`pdf.js`), requête `oeuvresPourCatalogue` (exclut archivées/retirées). Vignette : n° inv · titre · dimensions · médium ; ligne du bas = **prix si disponible, sinon « Non disponible »** (réservée = non disponible). **Reste** : Annexe A **dépôt** (rouge) + **retrait** (bleu `#2a5c8a`).
